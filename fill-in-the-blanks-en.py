...
'___3___': 'True',
'___4___': 'False'}
quiz_hard = {'topic': '''Python provides a simpler way to loop through elements
of the ___2___, and that's called a ___1___-loop. The structure of a
___1___-loop is like this. We have the keyword ___1___ followed by a name, and
this is a new name  ___1___ a variable that we can introduce, then the keyword
in followed by a ___2___,  and this is any expression which evaluates to a
___2___ followed by a ___3___. So this  is quite similar to what we've seen
___1___ the structure of a while loop and an if  statement with a ___4___
inside the ___1___. What a ___1___-loop like this means is  that ___1___ each
element in the ___2___ we're going to assign that element to the name  and
evaluate the ___4___.''',
'___1___': 'for',
'___2___': 'list',
'___3___': 'colon',
'___4___': 'block'}
...
def choose_level():
	'''Player choose game level and return the selected quiz'''
	while True:
...
def error_times_max():
	'''Player choose the max number of the error times
	and return this number as the limit of error times'''
	while True:
...
def replace(quiz, blank):
	'''Input the current question and the blank number,
...
def guess(quiz, blank_num, error_max):
	'''passing in the quiz, the blank number and the max wrong time,
	players input the answer to one blank,
	judge the answer and return the wrong times'''
	error_num = 0
	while error_num <= error_max:
		answer = raw_input('\nPlease type in '+ blank_num +' possible words:\n')
